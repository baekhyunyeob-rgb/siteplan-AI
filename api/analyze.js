...
출력 형식 (JSON):
{
  ...
  "보조금": [
    ...
    // 목적별 관련 단가 블록 생성
    const priceBlock = buildPriceBlock(purpose)
...
3. 예산은 아래 제공된 표준단가를 활용하여 공종별로 산출하세요.
   - "지붕 교체 약 ○○㎡ × 55,000원/㎡ = 약 ○○만원" 형식으로 근거를 제시하세요.
   - 면적은 건축물대장 또는 사용자 입력 기준으로 추정하세요.
   - 단가 출처가 '시장조사'인 항목은 ±30% 오차 가능성을 명시하세요.
4. 보조금은 해당 목적(${purpose})과 지역에 실제 존재 가능한 것만 제시하세요.
5. 반드시 JSON 형식으로만 응답하세요. 마크다운 코드블록 없이 순수 JSON만 출력하세요.`

    const userPrompt = `${landBlock}

## 참고 표준단가 (공종별 산출 근거로 활용)
${priceBlock}
...
// ─────────────────────────────────────────────────────────────────
// 목적별 단가 블록 생성 (프롬프트용)
// 표준품셈: 원가 × 1.35 (이윤+관리비+간접비 35% 가산)
// 시장시공가격/시장조사: 이윤 포함된 시장가 그대로 사용
// ─────────────────────────────────────────────────────────────────
function buildPriceBlock(purpose) {
  const PRICE = {
    리모델링: {
      지붕: [
        { 공종: '칼라강판 지붕 교체 (철거 포함)', 단위: '㎡', 단가: 55000, 출처: '시장조사', 오차: '±15~20%' },
        { 공종: '슬레이트 철거', 단위: '㎡', 단가: 24300, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '우레탄 도막방수 (3mm)', 단위: '㎡', 단가: 32300, 출처: '시장시공가격', 오차: '±10%' },
      ],
      외벽: [
        { 공종: '외벽 도장 (탄성 방수도료)', 단위: '㎡', 단가: 25000, 출처: '시장조사', 오차: '±15~20%' },
        { 공종: '외벽 단열 (EPS 100mm+마감)', 단위: '㎡', 단가: 65000, 출처: '시장조사', 오차: '±15~20%' },
        { 공종: '내벽 무늬코트', 단위: '㎡', 단가: 11000, 출처: '시장시공가격', 오차: '±10%' },
      ],
      창호: [
        { 공종: 'PVC 이중창 교체 (복층유리)', 단위: '㎡', 단가: 280000, 출처: '시장조사', 오차: '±15~20%' },
        { 공종: '현관문 교체 (단열 방화문)', 단위: '개소', 단가: 650000, 출처: '시장조사', 오차: '±20%' },
      ],
      내부: [
        { 공종: '석고보드 천장 마감', 단위: '㎡', 단가: 47300, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '바닥 타일 교체 (철거 포함)', 단위: '㎡', 단가: 74300, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '내부 도배', 단위: '㎡', 단가: 15000, 출처: '시장조사', 오차: '±15~20%' },
      ],
      설비: [
        { 공종: '기름보일러 교체', 단위: '식', 단가: 2500000, 출처: '시장조사', 오차: '±20%' },
        { 공종: '화장실 전면 개수', 단위: '식', 단가: 4500000, 출처: '시장조사', 오차: '±20%' },
        { 공종: '전기 배선 교체', 단위: '식', 단가: 5000000, 출처: '시장조사', 오차: '±20%' },
      ],
    },
    농지정리: {
      토공: [
        { 공종: '기계 절토', 단위: '㎥', 단가: 11500, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '기계 성토+다짐', 단위: '㎥', 단가: 9500, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '흙 운반 (5km 이내)', 단위: '㎥', 단가: 16200, 출처: '표준품셈×1.35', 오차: '±10%' },
      ],
      배수: [
        { 공종: '콘크리트 배수로 U형 300mm', 단위: 'm', 단가: 114800, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '법면 보호 코아네트+씨드', 단위: '㎡', 단가: 16800, 출처: '시장시공가격', 오차: '±10%' },
      ],
      포장: [
        { 공종: '농로 포장 아스콘 50mm', 단위: '㎡', 단가: 60800, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '농로 포장 콘크리트 150mm', 단위: '㎡', 단가: 74300, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '옹벽 블록 h=1m', 단위: 'm', 단가: 180000, 출처: '시장조사', 오차: '±15~20%' },
      ],
    },
    신축: {
      토공: [
        { 공종: '기계 터파기 토사', 단위: '㎥', 단가: 11500, 출처: '표준품셈×1.35', 오차: '±10%' },
        { 공종: '되메우기+다짐', 단위: '㎥', 단가: 8100, 출처: '표준품셈×1.35', 오차: '±10%' },
      ],
      참고단가: [
        { 공종: '농촌 단층 주택 목조경량', 단위: '평', 단가: 600000, 출처: '시장조사', 오차: '±20%' },
        { 공종: '농촌 단층 주택 조적조', 단위: '평', 단가: 500000, 출처: '시장조사', 오차: '±20%' },
      ],
    },
  }

  const data = PRICE[purpose] ?? PRICE['리모델링']
  const lines = [
    '[표준단가 기준: 2025년 상반기 / VAT 별도 / 전국 평균]',
    '[표준품셈×1.35: 공공 원가에 이윤·관리비·간접비 35% 가산한 견적가]',
  ]
  ...
  lines.push('\n※ 예산 산출 시 "지붕 교체 ○○㎡ × 단가 = ○○만원" 형식으로 공종별 근거를 제시하세요.')
  lines.push('※ 인근 지방 중소도시 업체가 수도권 대비 10~20% 유리. 복수 견적 비교 권장.')
  return lines.join('\n')
}
